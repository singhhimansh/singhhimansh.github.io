                ...
                { projects.map((project)=>{
                    ...
                    return(
                        <div key={project.id} className={`my-24 md:my-32 lg:mx-10 h-96 relative md:grid md:grid-cols-12 gap-3 items-center select-none ${ project.id % 2 === 1 ? "text-right" : "text-left" } `}>
                           ...
                           <div className={`z-[1] p-5 md:p-0 h-[90%]  bg-deepocean/90 md:bg-transparent absolute top-0 left-0 md:relative flex flex-col justify-center border md:border-none rounded-md border-emerald-200/40 w-full  ${ project.id % 2 === 1 ? "md:col-span-8  lg:col-span-6 md:col-end-13 lg:col-end-13" :"md:col-span-8 lg:col-span-6 md:col-start-1 "  }`}>
                                <h1 className="font-bold tracking-wider text-2xl md:text-3xl text-slate-400 md:text-emerald-200/60 hover:text-emerald-300 duration-200">{project.title}</h1>
